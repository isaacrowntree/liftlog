...
export function SetRow({
  ...
  return (
    ...
    >
      ...
      {timed ? (
        <input
          className="setfield" autoComplete="off" autoCorrect="off" spellCheck={false} onFocus={(e) => e.currentTarget.select()}
          type="number"
          inputMode="numeric"
          enterKeyHint="done"
          ...
          {!bodyweight && (
            <input
              ...
              onKeyDown={(e) => {
                ...
                const list = inputs ? [...inputs] : [];
                const next = list[list.indexOf(e.currentTarget) + 1];
                if (next) next.focus();
