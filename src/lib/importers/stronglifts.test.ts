...
describe("parseStrongLiftsCsv", () => {
  ...
  it("classifies always-zero-weight exercises as bodyweight", () => {
    const csv = CSV + '\n"2026/04/30","1269","Workout B","","97.75","Push Ups","3x10","","10x0","","30","","","0.6","7:00 AM","7:35 AM","","10","0","10","0","10","0","",""';
    const result = parseStrongLiftsCsv(csv);
    const pushups = result.workouts[1].exercises.find((e) => e.name === "Push Ups")!;
    expect(pushups.kind).toBe("bodyweight");
    expect(pushups.sets[0]).toEqual({ reps: 10 });
    // assisted work stays weighted (negative kg is signal, not absence)
    const chinups = result.workouts[1].exercises.find((e) => e.name === "Chinups")!;
    expect(chinups.kind).toBe("weighted");
  });
