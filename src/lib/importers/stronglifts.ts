...
export function parseStrongLiftsCsv(text: string): ImportResult {
  ...
  // Kind inference: StrongLifts has no explicit set kinds. An exercise whose
  // every set across the whole export is 0kg is bodyweight (push ups);
  // negative weights are assisted work and stay weighted.
  const everZeroOnly = new Map<string, boolean>();
  for (const w of workouts) {
    for (const e of w.exercises) {
      const allZero = e.sets.every((s) => (s.weightKg ?? 0) === 0);
      everZeroOnly.set(e.name, (everZeroOnly.get(e.name) ?? true) && allZero);
    }
  }
  for (const w of workouts) {
    for (const e of w.exercises) {
      if (everZeroOnly.get(e.name)) {
        e.kind = "bodyweight";
        e.sets = e.sets.map(({ reps }) => ({ reps }));
      }
    }
  }
