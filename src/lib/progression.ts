...
export function nextWorkingWeight(
  ...
  if (fails >= rule.deloadAfterFails) {
    // pct of |weight| SUBTRACTED, so a deload always makes the lift easier.
    // Negative weight = machine assistance: easier means more negative
    // (weight × (1 − pct) would move toward zero and get harder).
    return roundToIncrement(
      last.weightKg - Math.abs(last.weightKg) * rule.deloadPct,
      2.5,
    );
  }
